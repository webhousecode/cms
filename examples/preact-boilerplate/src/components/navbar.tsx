...
export function Navbar({ siteTitle, navLinks }: NavbarProps) {
  ...
  return (
    <header class="border-b border-border bg-card">
      ...
      {/* Mobile nav */}
      {menuOpen && (
        <nav class="border-t border-border px-4 pb-4 md:hidden">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              onClick={() => setMenuOpen(false)}
              class="block py-2 text-sm text-muted hover:text-foreground"
            >
              {link.label}
            </a>
          ))}
...
function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggle}
      class="rounded-md p-2 text-muted hover:text-foreground transition-colors"
