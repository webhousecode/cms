# Next.js GitHub Boilerplate — AI Builder Instructions

This is a **Next.js site** with GitHub-backed CMS storage. The CMS admin pushes content changes via signed webhook. The site receives updates in real-time via LiveRefresh SSE.
...
**Fly.io (recommended)** — Dockerfile included, deploy via CMS admin Deploy tab or CLI:
```bash
flyctl deploy --remote-only --ha=false
```
The Dockerfile uses Next.js standalone output. `fly.toml` is auto-generated by CMS admin (region: arn, port: 3000, 512MB RAM).

**Other providers:**
- **Vercel** — `npx vercel` (zero config)
- **Netlify** — `npm run build`, publish `.next`
- **CMS Admin** — Deploy tab auto-detects Dockerfile and deploys to Fly.io
