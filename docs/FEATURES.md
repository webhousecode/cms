...
| F75 | [AI Site Builder Guide](#f75-ai-site-builder-guide) | **Done** | [docs/features/F75-ai-site-builder-guide.md](features/F75-ai-site-builder-guide.md) |
...
**Phase 1 (Done):** Split the 2421-line monolithic `packages/cms/CLAUDE.md` into 21 focused module files hosted on GitHub raw. Slim index document (~180 lines) replaces the monolith. AI sessions fetch modules on demand. **Phase 2 (Done 2026-04-16):** Dedicated AI Builder Site at `docs.webhouse.app/ai` (future subdomain `ai.webhouse.app`) with a self-guided Step 0-9 walkthrough (`_walkthrough.md`) — scaffold → config → content → deploy — written for AI coding assistants (Claude Code, Cursor, Copilot, Gemini, Windsurf). Serves `text/markdown` at a pretty URL, all 21 modules at `/ai/{slug}`, plus machine-readable `/ai/llms.txt`, `/ai/manifest.json`, `/ai/index.json`. Dogfoods docs.webhouse.app (built with @webhouse/cms). `X-Robots-Tag: noindex` on all responses. Zero new infra — route group on existing docs site.
