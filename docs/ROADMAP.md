...
| 11d | **F139 Headless Site API & Chat Embedding** | Small | Use CMS Admin as headless backend inside your own Next.js (or any framework) site. Auth via wh_ Access Token. Read/write content, trigger deploys, read form inboxes, embed AI chat in custom design. AI builder guide module 22 + bilingual docs page. Chat embedding lets sites surface same tools/model as Admin chat with custom styling. |
| 11e | **F140 Empty-Org UX Regression** | Small | Empty orgs (0 sites) strip user-menu and org-switcher chrome that should remain available — gravatar, Org Settings, "+ New org", "All orgs" all disappear. Reproduces on prod + localhost. Hoist user-menu out of site-context-aware path so empty orgs keep full header. Caught during sanne-andersen migration when site-move into a new empty org locked the user out of Site Settings. |
| 11f | **F141 Site Switch Context Leak** | Small-Medium | Switching active site updates header chrome but not sidebar/content — UI shows site B but content listing comes from site A's config. Caught when sanne-andersen picker selection still showed webhouse.dk Case Studies. Likely missing `router.refresh()` after switch, or stale CMS config cache. Critical first check: verify API writes don't leak across sites (security incident if so). |
| 11g | **F143 Common Build Server** | Small-Medium | Quick-path response til 2026-05-02 trail-landing 5-time Beam-saga. Pre-installerede core-deps i cms-admin (marked, sharp, gray-matter, slugify, marked-highlight) + on-demand pnpm install for sites med extra deps. Bevarer `build.ts` som kontrakt, men flytter execution til cms-admin's egen Node-proces. Per-site `node_modules` forsvinder. Filesystem-sites kan deployes fra ANY cms-admin (lokal eller webhouse.app). 3-5 dage. Sister: F142. |
| 11h | **F142 Templated SSG Runtime** | Medium | Langsigtet svar på samme problem som F143. Erstatter build.ts med deklarativ `templates/`-mappe (tagged-template-literal funktioner). 99% af eksisterende sites kan migreres (audit: 11/20 har 0 deps, 5/20 har kun marked). 7-10 dage. AI/dev sessions skriver mindre kode pr site. Custom build.ts forbliver escape hatch via F143. |
| 11i | **F144 Dynamic Site Build Orchestrator** | Medium | Generaliserer F143's build-server-pattern til SSR sites (Next.js, Bun/Hono). cms-admin spawner ephemeral Fly Machines som build-VM'er, builder Docker image, pusher til GHCR, orchestrerer rolling restart af target Fly app. 3 triggers: manual rocket / site-repo's GHA / GitHub webhook. Erstatter "flyctl deploy fra terminalen" med knap i admin. ~$0.01 per Next.js deploy. 4-6 dage forudsat F143 landed. |
| 11j | **F145 ICD — Instant Content Deployment** | Small (polish) | Formaliserer det 3. ben i deploy-triumviratet (ICD + F143 + F144). Core er allerede shipped (HMAC-signeret POST → site's /api/revalidate ved content-edit, sub-sekund propagation). F145 leverer: dedikeret UI, secret rotation, retry-logik, batched revalidation, idempotency keys, kanonisk boilerplate /api/revalidate, AI guide + docs. ~3-4 dage polish. |
| 11k | **F146 URL-Based Site Routing** | Medium | Flyt active-site fra cookie til URL-path (`/admin/{slug}/...`). Løser parallel-tab cookie tug-of-war, gør bookmarks stabile per-site, sharing-links lander det rigtige sted. ~2.5 dage. Small bridge `/admin/switch/<slug>` allerede shipped. |
| 11l | **F147 Webapp Blueprint Contract** | Large (foundational) | Gør broberg.ai-stack'en (cms, trail, stripe-connect, MCP) til default backend for alle FUTURE customer webapps via contract + scaffolder + AI Builder Guide module. Forhindrer cc-sessions i at rulle deres egen auth/content/chat (sanneandersen-mønstret). Eksponerer F59 som public auth API. Implementation-agnostic env-vars enabler senere F148 (Web Application Server gateway). ~7 dage core, P7 (sanne migration runbook) deferred. |
| 11m | **F148 Web Application Server (broberg-app)** — REBORN as F149 P10 | Small (3d, depends on F149 P2-P7) | Original from-scratch design afvist 2026-05-09 (blast radius). Reborn 2026-05-10 som thin Hono service der mounter F149-packages' server-routes som HTTP-endpoints. Customer-webapps vælger Mode A (SDK direct) eller Mode B (App Server). SPOF strukturelt løst — Mode A virker altid uafhængigt. |
| 11n | **F149 Web App SDK (`@webhouse/*`)** | XL (foundational) | 13 vedligeholdte npm-pakker (`@webhouse/auth, db, users, entity-store, mail, notifications, logs, webhooks, stripe, files, cms-content, trail-connect, chat, mcp, ai`) der wrapper cross-cutting concerns alle customer-webapps i dag duplikerer. Hvert site vælger egen runtime + DB. Zero shared SPOF. Scaffolding-site med ægte testdata som cc-reference. ~35-45 dage MVP across 8 phases. Erstatter F148; implementerer F147-kontrakten via biblioteker. |
