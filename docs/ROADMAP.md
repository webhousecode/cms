...
| 11m | **F148 Web Application Server (broberg-app)** | Large (foundational) | Multi-tenant Bun-Hono service på `app.broberg.ai` der implementerer F147-kontrakten serverside med ægte tovejs-Postgres for transaktionel state (bookings, orders, sessions), passkey-auth (F59 port), Stripe Connect routing, generic entity-store med RLS pr. site_id. Mirror'er auth_users til CMS users-collection via webhook — løser sanneandersen drift permanent. Stack: Bun + Hono + Drizzle + Neon Postgres + Tigris + Fly multi-region. ~11 dage MVP. Skal komme EFTER F147 (kontrakt først, gateway efter). Open decisions awaiter Christian. |
