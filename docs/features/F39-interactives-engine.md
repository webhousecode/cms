...
- [x] Define data collections pattern in CLAUDE.md (full documentation with examples)
- [x] Data-driven pattern documented: CMS Collection → getDocument() → Page → Component props
- [ ] ~~Build Interactive component loader~~ — not needed; pattern is documented for AI site builders to implement per-site
- [x] SproutLake infographics refactored: reads `interactiveFile` from CMS, legacy mapping as fallback
...
- [x] "Create with AI" — modal with prompt input, AI generates standalone HTML, creates as draft interactive
- [x] AI system prompt instructs generation of complete standalone HTML with inline CSS/JS
- [ ] ~~Ints generated by agents land in Curation Queue~~ — deferred; direct creation as draft is simpler and sufficient
