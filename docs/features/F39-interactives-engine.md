...
1. Create `interactives` storage system (files in `_data/interactives/` or upload dir)
2. Build Interactives Manager page (`/admin/interactives`)
3. Upload HTML files, list with previews
4. Preview mode (iframe with `src` pointing to uploaded file)

### Phase 2: Editing
5. Visual Edit mode (wysiwyg-inject from Pitch Vault, adapted for CMS admin)
6. AI Edit mode (reuse existing AI chat component)
7. Code View mode (textarea with syntax highlighting)

### Phase 3: Data Separation
8. Define data collections pattern in CLAUDE.md
9. Build Interactive component loader that passes CMS data as props
10. Convert SproutLake infographics to data-driven Interactives

### Phase 4: Embedding
11. TipTap `interactiveEmbed` node (toolbar button, picker, preview)
12. Block type `interactive` for blocks fields
13. Standalone page rendering (collection with interactive field)

### Phase 5: AI Generation
14. "Create with AI" — prompt → AI generates Interactive HTML
15. AI understands CMS data collections and generates data-reading code
