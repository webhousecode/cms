...
- **Pre-warm:** on invalidation (debounced ~5s) regenerate the content-dependent entries in the background; a cold peek on any known key also schedules a one-shot regen (deduped) so the next click is instant. Guarded on the service token / CMS_JWT_SECRET being present → ship-dark (no crash, endpoint just returns cached:false) when unconfigured.
  - **Boot pre-warm: deliberately NOT implemented.** The cache file lives on the Fly volume (webhouse-app is filesystem-stateful, single machine) so warm answers survive restarts; staleness is covered by the TTL + lazy-regen. Eagerly regenerating every tenant's 4 answers on each boot would hammer the model for no user-visible gain. Revisit only if the store moves off the persistent volume (F152 HA).
