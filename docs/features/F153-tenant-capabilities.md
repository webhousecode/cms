...
> **Status: F153.1 + F153.2 + F153.3 SHIPPED — feature complete.** F153.1 = foundation + Features settings UI (all default-ON). F153.2 = gates for the five headline capabilities (`ai`/`agents`/`chat` + `seo`/`maps`/`interactives`) on every layer. F153.3 = the remaining four groups (`forms`/`scheduling`/`quality`/`backup`), **org→site inheritance** (org sets defaults, sites override), and a **Minimal-by-default onboarding** (new sites start stripped; existing sites untouched). All runtime-proven, all backward-compatible. §9 decisions resolved: coarse groups, site+org levels, admin-gated (`capabilities.manage`), new-site default = Minimal. Only optional follow-up left is F153.4 (tie a profile to a pricing tier).
...
- **F153.2 — Wire the headline gates:** AI (nav + editor AI buttons + `ai.*` API routes + chat + agents), then SEO, Maps, Interactives. Each gate = capability ∧ permission. Verify a "Minimal"-profile tenant in the browser (via Lens) shows the stripped CMS.
  ...
  - **SEO / Maps / Interactives — SHIPPED.** `seo`: sidebar SEO + Visibility nav, redirect layouts for `/admin/seo` + `/admin/visibility`, server gates on `/api/admin/seo/{optimize-bulk,og-image,keywords}`. `maps`: the "insert map" embed button in the richtext media menu. `interactives`: sidebar nav, redirect layout for `/admin/interactives*`, server gates on `/api/interactives` (POST/PUT/DELETE) + `[id]/translate`, and the interactive AI panel gated on `ai`. Command-palette entries (nav-seo, nav-visibility, nav-ints) carry their `capability`. Runtime-proven: `seo` off → optimize-bulk 404; `interactives` off → translate 404. All AI routes confirmed on @broberg/ai-sdk (no direct provider SDK anywhere in cms-admin).
  - **F153.2 complete.** Remaining capability groups (forms, scheduling, quality, backup) + org-inheritance move to F153.3.
- **F153.3 — Remaining groups + org inheritance + onboarding profile pick** (new tenant/onboarding chooses Minimal/Standard/Full).
  - **Remaining groups (forms / scheduling / quality / backup) — SHIPPED.** Now every capability in the catalog gates its surface. `forms`: nav + redirect layout for `/admin/forms` + `/api/admin/forms` (GET+POST); public `/api/forms/*` (submission/widget) intentionally NOT gated. `scheduling`: Calendar nav + redirect layout for `/admin/scheduled` + `/api/cms/scheduled`; the cron publisher `/api/publish-scheduled` NOT gated. `quality`: Link Checker + Lighthouse + AI Analytics nav, redirect layouts for `/admin/{link-checker,lighthouse,performance}`, server gates on `/api/check-links` (POST) + `/api/admin/lighthouse/scan` (POST). `backup`: nav (admin ∧ backup) + redirect layout + `/api/admin/backups` (POST) + `[id]` (POST/DELETE). Command-palette nav-calendar/nav-analytics/nav-linkchecker/nav-backup/act-backup carry `capability`. Runtime-proven: each off → its route 404; restore → 200.
  - **Org-level inheritance — SHIPPED.** `mergeConfigs` now deep-merges `capabilities` (org defaults ← site overrides) instead of shallow-replacing, so a site can flip one feature back on without dropping the org's other toggles. `OrgSettings.capabilities` added; Organization Settings → **Features** tab (reuses `CapabilitiesSettingsPanel` with `endpoint="/api/admin/org-settings"` + `selfSave`). Runtime-proven: org `ai:false` → site inherits `{ai:false}`; site `ai:true` → override wins. (§9.2 = org-default + site-override.)
  - **Onboarding default = Minimal — SHIPPED.** A brand-new blank site (`/api/cms/registry` add-site) is seeded with `CAPABILITY_PROFILES.minimal` (content + media only) via `flag:"wx"` (never clobbers). Existing sites untouched (no stored caps → all-on); clone/beam/wp-import paths unaffected. (§9.4 = Christian chose Minimal.) Admins enable features in Site Settings → Features.
  - **F153.3 complete.** Capabilities are now a full per-tenant axis: catalog + profiles (F153.1), gates on every layer for all 10 groups (F153.2 + F153.3), org→site inheritance, and a stripped-by-default onboarding. Future (optional) F153.4: tie a profile to a pricing tier.
