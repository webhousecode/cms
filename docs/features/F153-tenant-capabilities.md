...
> **Status: F153.1 + F153.2 SHIPPED.** F153.1 (foundation + Features settings UI) shipped, all-default-ON. F153.2 wired the gates for all five headline capabilities — `ai` / `agents` / `chat` (AI phase) plus `seo` / `maps` / `interactives` — across every layer (nav, editor controls, command-palette, model-invoking API routes, server redirect layouts). All runtime-proven, all default-ON (zero change for current tenants). Decisions still open are collected in **§9** (the architecture chosen uses the recommended answers). Next: F153.3 — remaining groups (forms/scheduling/quality/backup) + org-level inheritance + onboarding profile pick.
...
- **F153.3 — Remaining groups + org inheritance + onboarding profile pick** (new tenant/onboarding chooses Minimal/Standard/Full).
  ...
  - **Remaining for F153.3: org-level inheritance + onboarding profile pick** — open (needs the §9.2 / §9.4 calls).
