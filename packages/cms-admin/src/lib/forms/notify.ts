 ...
 */
async function resolveMailer(overrideFrom?: string): Promise<{ apiKey?: string; from: string }> {
  ...
  return { apiKey, from: buildFrom(fromName, fromEmail) };
}

/** Shared branded shell — dark card, gold accent bar — matching
 *  lib/email.ts's renderInviteEmail so every cms-sent email looks the same. */
function wrapBrandedEmail(opts: { title: string; bodyHtml: string; footerNote?: string }): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="text-align:center;padding:24px 0 0;">
      ...
      <p style="margin:0;font-size:10px;color:#333;">Sent by webhouse.app</p>
    </div>
  </div>
</body>
</html>`;
...
async function sendEmailNotification(form: FormConfig, sub: FormSubmission): Promise<void> {
  ...
  const html = wrapBrandedEmail({ title: `New ${form.label} submission`, bodyHtml });
...
async function sendAutoReply(form: FormConfig, sub: FormSubmission): Promise<void> {
  ...
  const linksHtml = links.length
    ? `<div style="margin-top:28px;padding-top:24px;border-top:1px solid #262626;">
         ...
         ${links.map((l) => `<a href="${escHtml(l.url)}" style="display:block;margin:0 0 10px;font-size:14px;color:#F7BB2E;text-decoration:none;">${escHtml(l.label)} →</a>`).join("")}
       </div>`
    : "";
  ...
  const bodyHtml = `
    ...
    ${linksHtml}
  `;
  const html = wrapBrandedEmail({ title: subject, bodyHtml });
