 ...
 *
 * Provider-agnostic image analysis via the central @broberg/ai-sdk facade
 * (ai.vision for images, ai.chat for SVG-source text). Uses whichever provider
 ...
 *
 * API keys come from existing ai-config.json or env vars (resolved by getAI()).
 */

import { z } from "zod";
import { readAiConfig } from "@/lib/ai-config";
import { getAI, parseJsonLoose } from "@/lib/ai/client";
import type { TierSpec } from "@broberg/ai-sdk";
...
/** Append a JSON-shape instruction so vision/chat replies parse deterministically. */
function withJsonShape(prompt: string, shape: string): string {
  return `${prompt}${JSON_OUTPUT_RULE}\nShape: ${shape}`;
}
...
/** Resolve the best available vision route (provider+model) from configured keys. */
...
export async function analyzeImage(
  ...
  const { override, provider } = await getVisionSpec();

  const { text } = await ai.vision({
    image: new Uint8Array(imageBuffer),
    mimeType,
    prompt: withJsonShape(
      getAnalysisPrompt(language),
      '{"caption": string, "alt": string (max 125 chars), "tags": string[] (3-8 items)}',
    ),
    tier: "vision",
    override,
    purpose: "media.image-analysis",
  });

  const object = imageAnalysisSchema.parse(parseJsonLoose(text));
...
- Do not use generic tags like "image" or "photo".`;
  ...
  const { text } = await ai.vision({
    ...
    prompt: withJsonShape(
      prompt,
      `{"captions": {<locale>: string} for ${locales.join(", ")}, "alts": {<locale>: string}, "tags": string[]}`,
    ),
    tier: "vision",
    override,
    purpose: "media.image-analysis",
  ...
  const object = multiSchema.parse(parseJsonLoose(text));
  return {
...
export async function analyzeSvgText(
  ...
  const { text } = await ai.chat({
    ...
    messages: [{
      role: "user",
      content: withJsonShape(
        `You are an image analyst for a CMS media library. This is an SVG file — its XML source is below. Analyze the structure (shapes, paths, text elements, viewBox, IDs, classes) to infer what the SVG depicts.
...
export async function testVisionConnection(): Promise<{ ok: boolean; provider?: string; error?: string }> {
  try {
    ...
    const { text } = await ai.chat({
      ...
      purpose: "media.vision-test",
    });

    const parsed = z.object({ status: z.string() }).parse(parseJsonLoose(text));
    return { ok: parsed.status === "ok", provider };
  } catch (err) {
