...
export async function GET() {
  try {
    ...
    // Completed scheduled backup events
    try {
    ...
    // Future scheduled backup events (next 30 days)
    ...
    // Future scheduled link-check events (next 30 days)
    ...
    return NextResponse.json({ events, calendarToken, orgId, siteId });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to load scheduled events" },
      { status: 500 },
    );
  }
}
