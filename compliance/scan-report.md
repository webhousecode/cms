# F201.1 — scanningsrapport

Kørt 2026-09-26T09:16:13.176Z · 39 af 40 projekter scannet.

## Projekter

| projekt | repo | status | commit | filer | Fly-regioner |
|---|---|---|---|---|---|
| ai-sdk | broberg-ai/ai-sdk | scannet | a8876f6 | 97 |  |
| anna | broberg-ai/annaslothart | scannet | 21c747a | 1 |  |
| autodoc | broberg-ai/autodoc | scannet | 8fab60c | 52 | broberg-autodoc=arn |
| beacon | broberg-ai/beacon | scannet | 94de1c6 | 175 |  |
| broberg-ai-site | broberg-ai/broberg-ai-site | scannet | 2720df0 | 107 | broberg-ai=arn |
| broberg-id | broberg-ai/broberg-id | scannet | 8a682cf | 111 | broberg-id=arn |
| buddy | webhousecode/buddy | scannet | 6c8cf0f | 690 | buddy-brain=arn, buddy-edge-fly=arn, webhouse-old=arn, buddy-voice=arn, buddy-cloud=arn |
| cardmem | broberg-ai/cardmem | scannet | 6b34b87 | 1028 | cardmem-lens=arn, cardmem=arn |
| cms | webhousecode/cms | scannet | 18b6cad | 1016 | webhouse-cms=arn, webhouse-app=arn |
| components | broberg-ai/components | scannet | 7d36015 | 415 | broberg-discovery=arn |
| contentpush | broberg-ai/contentpush | scannet | 8e66535 | 77 | contentpush=arn |
| contract-manager | broberg-ai/contract-manager | scannet | 4d72c97 | 177 | webhouse-contract-manager=arn |
| coverletter-generator | cbroberg/coverletter-generator | scannet | 90fd64f | 271 | coverletter-generator=arn |
| cronjobs | webhousecode/cronjobs | scannet | ca61281 | 143 | webhouse-cronjobs=arn |
| dns-mcp | webhousecode/dns-mcp | scannet | ccdee00 | 24 | dns-mcp=arn |
| fd-ishoej | broberg-ai/fd-ishoej | scannet | 3a50a68 | 1 |  |
| fd-sundhed | broberg-ai/fd-sundhed | scannet | 9b5c327 | 1165 | fd-sundhed=arn |
| fleet | broberg-ai/fleet | scannet | 888b59e | 18 |  |
| forager | broberg-ai/forager | scannet | 634cd3d | 56 |  |
| fysio-dk-aalborg | broberg-ai/fdaa | scannet | f17bb41 | 15 |  |
| fysiodk-aalborg-sport | webhousecode/fysiodk-aalborg-sport | scannet | b595275 | 431 | fd-sport=arn |
| happy-little-place | broberg-ai/happy-little-place | scannet | 31a990b | 67 | happy-little-place=arn |
| helpdesk | broberg-ai/helpdesk | scannet | ccf6644 | 234 | broberg-helpdesk-db=arn, broberg-helpdesk-api=arn |
| how | broberg-ai/house-of-wellness | scannet | f64c05e | 8 |  |
| kai | broberg-ai/kai | scannet | d014da5 | 2 |  |
| lens-fixture | — | **IKKE scannet:** intet repo registreret i cardmem |  |  |  |
| mailworker | broberg-ai/mailworker | scannet | 045a020 | 71 | mailworker-api=arn |
| moovyy | cbroberg/moovyy | scannet | 457fdc4 | 211 | moovyy=arn |
| notesmem | broberg-ai/notesmem | scannet | ae73b28 | 29 |  |
| openbuddy | broberg-ai/openbuddy | scannet | 02f6c04 | 16 |  |
| pitch | cbroberg/pitch | scannet | d78133f | 270 | pitch-vault=arn |
| sanneandersen | webhousecode/sanneandersen | scannet | 87eb9b0 | 976 | sanneandersen-site=arn |
| storeform | broberg-ai/storeform | scannet | 70bd53a | 24 |  |
| super | broberg-ai/super-agent | scannet | 7d41eeb | 66 |  |
| trail | broberg-ai/trail | scannet | 6c0901f | 853 | trail-admin=arn, trail-db-001=arn, trail-landing=arn, trail-engine-001=arn, trail-widget=arn, trail-engine=arn |
| upmetrics | broberg-ai/upmetrics | scannet | 71924d0 | 140 | upmetrics=arn |
| vn-leker | webhousecode/vnlekerv2 | scannet | bc73500 | 13 |  |
| voice-engine | broberg-ai/voice-engine | scannet | 914a5e4 | 66 | broberg-services=arn |
| whop | webhousecode/whop | scannet | 2d231ca | 103 | webhouse-whop=arn |
| xrt81-com | broberg-ai/xrt81 | scannet | f7b8622 | 329 | xrt81=arn |

## Leverandører

| leverandør | produkter |
|---|---|
| Fly.io | ai-sdk, autodoc, broberg-ai-site, broberg-id, buddy, cardmem, cms, components, contentpush, contract-manager, coverletter-generator, cronjobs, dns-mcp, fd-sundhed, fysiodk-aalborg-sport, happy-little-place, helpdesk, mailworker, moovyy, pitch, sanneandersen, trail, upmetrics, voice-engine, whop, xrt81-com |
| Tigris | cardmem, trail |
| Cloudflare | broberg-ai-site, buddy, cardmem, cms, components, contract-manager, coverletter-generator, dns-mcp, fd-sundhed, helpdesk, notesmem, sanneandersen, upmetrics, xrt81-com |
| Resend | broberg-ai-site, broberg-id, cardmem, cms, components, contract-manager, cronjobs, fd-sundhed, happy-little-place, helpdesk, moovyy, pitch, sanneandersen, trail, upmetrics, vn-leker, whop, xrt81-com |
| Stripe | cardmem, cms, components, mailworker, sanneandersen |
| Supabase | cardmem, cms, fd-sundhed, fysiodk-aalborg-sport, whop |
| Turso | buddy, cardmem, components, contentpush, happy-little-place, helpdesk, super, trail, xrt81-com |
| Amazon Web Services | cms, forager, sanneandersen, trail, whop |
| Mistral AI | ai-sdk, broberg-ai-site, buddy, cardmem, cms, components, contentpush, coverletter-generator, fd-sundhed, fysiodk-aalborg-sport, helpdesk, pitch, trail, xrt81-com |
| OpenAI | ai-sdk, buddy, cardmem, cms, trail, xrt81-com |
| Anthropic | ai-sdk, buddy, cardmem, cms, coverletter-generator, fd-sundhed, fysiodk-aalborg-sport, happy-little-place, trail, whop, xrt81-com |
| Google (Gemini/Vertex AI) | ai-sdk, buddy, cardmem, cms, components, super |
| DeepInfra | ai-sdk, pitch |
| OpenRouter | ai-sdk, buddy, cardmem, components, sanneandersen, trail, upmetrics, xrt81-com |
| DeepSeek | ai-sdk, buddy, cardmem, components |
| ElevenLabs | ai-sdk, buddy, cardmem, cms, contentpush |
| Microsoft Azure | ai-sdk, broberg-ai-site, buddy, cardmem, contentpush |
| Black Forest Labs | ai-sdk, cardmem, contentpush |
| GitHub | ai-sdk, autodoc, broberg-id, buddy, cardmem, cms, components, contract-manager, coverletter-generator, dns-mcp, fd-sundhed, fleet, forager, fysio-dk-aalborg, fysiodk-aalborg-sport, happy-little-place, helpdesk, how, moovyy, pitch, sanneandersen, trail, upmetrics, voice-engine |
| Google (Workspace/Gmail/Calendar/OAuth) | ai-sdk, broberg-id, cardmem, cms, coverletter-generator, fd-sundhed, fysiodk-aalborg-sport, mailworker, moovyy, sanneandersen, trail, xrt81-com |
| Google Maps | cms |
| Google PageSpeed Insights | cms |
| Google Firebase (FCM) | cms, fd-sundhed, fysiodk-aalborg-sport |
| Apple (APNs/Sign in/App Store) | broberg-id, buddy, cardmem, fd-sundhed, fysiodk-aalborg-sport, moovyy, storeform, xrt81-com |
| Web Push (browserens push-tjeneste) | cardmem, cms, components, fd-sundhed, fysiodk-aalborg-sport, happy-little-place, moovyy, trail, xrt81-com |
| LinkedIn | buddy, cms, coverletter-generator, xrt81-com |
| Microsoft (login) |  |
| Simply.com | dns-mcp |
| pCloud | cms |
| GatewayAPI | broberg-id, components |
| Twilio | cardmem |
| Complimenta | fd-sundhed, fysio-dk-aalborg |
| Opkald.ai | fd-sundhed |
| Zoom | cardmem |
| Daily.co | buddy |
| LiveKit | buddy |
| OpenSubtitles | moovyy |
| SubDL | moovyy |
| TMDB | moovyy |
| Unsplash | cms |
| DAWA (Dataforsyningen) | xrt81-com |
| Upstash |  |
| Vercel | cardmem, cms |
| Netlify | cms |
| npm | ai-sdk, cms, components, coverletter-generator, dns-mcp, fleet, fysio-dk-aalborg, how, moovyy, pitch, sanneandersen, trail, upmetrics, vn-leker |
| SMS.dk | components |
| Verda (GPU-cloud) | voice-engine |
| Tailscale | buddy |
| Google Play | fd-sundhed, fysiodk-aalborg-sport, storeform |
| DeepL | ai-sdk, components |
| fal.ai | ai-sdk, cardmem, contentpush, sanneandersen, super |
| Hugging Face | fd-sundhed, forager, voice-engine |
| RunPod | trail, voice-engine |
| Requesty | ai-sdk |
| Tavily | cms |
| Brave Search | cms |
| Perplexity | ai-sdk |
| Bunny.net | cms |
| Backblaze B2 | cms |
| Scaleway | cms |
| Hetzner | cms |
| Forward Email | moovyy |
| inMobile | components |
| QuickPay | buddy |
| Polar | coverletter-generator, pitch |
| Flexprice | buddy |
| Piwik PRO | buddy |
| Google Analytics | buddy, xrt81-com |
| Slack | buddy, cardmem |
| Discord | buddy, cardmem, cms, contentpush, cronjobs, fd-sundhed, fysiodk-aalborg-sport, sanneandersen, upmetrics, whop |
| Gravatar | broberg-id, cms, components, coverletter-generator, fd-sundhed, fysiodk-aalborg-sport, pitch, trail |
| Vimeo | cardmem, cms, sanneandersen |

## Vurdering pr. leverandør (F201.2)

| leverandør | anbefaling | aftale | overførsel | helbredsdata uden for EU | link |
|---|---|---|---|---|---|
| Fly.io | **acceptér/underskriv** | skal_accepteres | DPF | JA (fd-sundhed, fysiodk-aalborg-sport, sanneandersen) | https://fly.io/documents/ |
| Tigris | **link** | auto | SCC |  | https://www.tigrisdata.com/docs/legal/data-processing/ |
| Cloudflare | **link** | auto | DPF+SCC | JA (fd-sundhed, sanneandersen) | https://www.cloudflare.com/cloudflare-customer-dpa/ |
| Resend | **link** | auto | DPF+SCC | JA (fd-sundhed, sanneandersen) | https://resend.com/legal/dpa |
| Stripe | **link** | auto | DPF+SCC | JA (sanneandersen) | https://stripe.com/legal/dpa |
| Supabase | **link** | auto | SCC | JA (fd-sundhed, fysiodk-aalborg-sport) | https://supabase.com/legal/dpa |
| Turso | **acceptér/underskriv** | skal_accepteres | ukendt |  | https://turso.tech/terms-of-use |
| Amazon Web Services | **link** | auto | DPF+SCC | JA (sanneandersen) | https://aws.amazon.com/compliance/gdpr-center/ |
| Mistral AI | **link** | auto | EU |  | https://legal.mistral.ai/terms/data-processing-addendum |
| OpenAI | **link** | auto | SCC |  | https://cdn.openai.com/pdf/openai-data-processing-addendum.pdf |
| Anthropic | **link** | auto | SCC | JA (fd-sundhed, fysiodk-aalborg-sport) | https://www.anthropic.com/legal/data-processing-addendum |
| Google (Gemini/Vertex AI) | **link** | auto | DPF |  | https://cloud.google.com/terms/data-processing-addendum |
| DeepInfra | **erstat/begræns** | mangler | ukendt |  | https://deepinfra.com/privacy |
| OpenRouter | **link** | auto | SCC | JA (sanneandersen) | https://openrouter.ai/data-processing-agreement |
| DeepSeek | **erstat/begræns** | mangler | ukendt |  | https://cdn.deepseek.com/policies/en-US/deepseek-open-platform-terms-of-service.html |
| ElevenLabs | **link** | auto | DPF+SCC |  | https://elevenlabs.io/dpa |
| Microsoft Azure | **link** | auto | DPF |  | https://www.microsoft.com/licensing/docs/view/Microsoft-Products-and-Services-Data-Protection-Addendum-DPA |
| Black Forest Labs | **acceptér/underskriv** | skal_accepteres | SCC |  | https://bfl.ai/legal/privacy-policy |
| GitHub | **link** | auto | DPF+SCC | JA (fd-sundhed, fysio-dk-aalborg, fysiodk-aalborg-sport, sanneandersen) | https://docs.github.com/en/site-policy/privacy-policies/github-data-protection-agreement |
| Google (Workspace/Gmail/Calendar/OAuth) | **acceptér/underskriv** | skal_accepteres | DPF+SCC | JA (fd-sundhed, fysiodk-aalborg-sport, sanneandersen) | https://knowledge.workspace.google.com/admin/compliance/privacy-compliance-and-records-for-google-workspace-and-cloud-identity |
| Google Maps | **ingen handling** | ikke_databehandler | DPF+SCC |  | https://mapsplatform.google.com/resources/trust-center/gdpr/ |
| Google PageSpeed Insights | **ingen handling** | ikke_databehandler | ukendt |  | https://developers.google.com/speed/docs/insights/v5/get-started |
| Google Firebase (FCM) | **link** | auto | DPF+SCC | JA (fd-sundhed, fysiodk-aalborg-sport) | https://firebase.google.com/terms/data-processing-terms |
| Apple (APNs/Sign in/App Store) | **ingen handling** | ikke_databehandler | ukendt |  | https://developer.apple.com/support/terms/apple-developer-program-license-agreement/ |
| Web Push (browserens push-tjeneste) | **ingen handling** | ikke_databehandler | ukendt |  | https://hacks.mozilla.org/2016/01/web-push-arrives-in-firefox-44/ |
| LinkedIn | **ingen handling** | ikke_databehandler | SCC |  | https://www.linkedin.com/legal/l/bd-dpa |
| Microsoft (login) | **ingen handling** | ikke_databehandler | DPF |  | https://learn.microsoft.com/en-us/entra/identity-platform/v2-overview |
| Simply.com | **acceptér/underskriv** | skal_accepteres | EU |  | https://www.simply.com/compliance/ |
| pCloud | **erstat/begræns** | mangler | ukendt |  | https://www.pcloud.com/business_agreement.html |
| GatewayAPI | **acceptér/underskriv** | skal_accepteres | EU |  | https://onlinecity.io/legal-documents/gatewayapi/dpa-eu |
| Twilio | **link** | auto | DPF+SCC |  | https://www.twilio.com/en-us/legal/data-protection-addendum |
| Complimenta | **ingen handling** | ikke_databehandler | ukendt |  | https://complimenta.com/support/privatlivspolitik/ |
| Opkald.ai | **acceptér/underskriv** | skal_accepteres | EU |  | https://opkald.ai/ |
| Zoom | **link** | auto | DPF+SCC |  | https://explore.zoom.us/docs/doc/Zoom_GLOBAL_DPA.pdf |
| Daily.co | **acceptér/underskriv** | skal_accepteres | DPF+SCC |  | https://www.daily.co/legal/data-processing-addendum/ |
| LiveKit | **link** | auto | DPF+SCC |  | https://livekit.com/legal/data-processing-addendum |
| OpenSubtitles | **ingen handling** | ikke_databehandler | ukendt |  |  |
| SubDL | **ingen handling** | ikke_databehandler | ukendt |  |  |
| TMDB | **ingen handling** | ikke_databehandler | ukendt |  | https://www.themoviedb.org/api-terms-of-use |
| Unsplash | **ingen handling** | ikke_databehandler | ukendt |  | https://unsplash.com/api-terms |
| DAWA (Dataforsyningen) | **ingen handling** | ikke_databehandler | EU |  | https://dawadocs.dataforsyningen.dk/dok/om |
| Upstash | **link** | auto | SCC |  | https://upstash.com/trust/dpa.pdf |
| Vercel | **acceptér/underskriv** | skal_accepteres | DPF+SCC |  | https://vercel.com/legal/dpa |
| Netlify | **link** | auto | DPF+SCC |  | https://www.netlify.com/gdpr-ccpa/ |
| npm | **ingen handling** | ikke_databehandler | ukendt |  | https://docs.npmjs.com/policies/privacy/ |
| SMS.dk | **acceptér/underskriv** | skal_accepteres | EU |  | https://sms.dk/sms-dk-er-gdpr-kompatibel/ |
| Verda (GPU-cloud) | **link** | auto | EU |  | https://verda.com/terms-and-conditions |
| Tailscale | **link** | auto | SCC |  | https://tailscale.com/dpa |
| Google Play | **ingen handling** | ikke_databehandler | SCC |  | https://play.google/developer-distribution-agreement.html |
| DeepL | **link** | auto | EU |  | https://www.deepl.com/en/pro-license |
| fal.ai | **link** | auto | SCC | JA (sanneandersen) | https://fal.ai/legal/data-processing-addendum |
| Hugging Face | **acceptér/underskriv** | skal_accepteres | SCC | JA (fd-sundhed) | https://huggingface.co/docs/inference-endpoints/en/security |
| RunPod | **acceptér/underskriv** | skal_accepteres | SCC |  | https://www.runpod.io/legal/data-processing-agreement |
| Requesty | **acceptér/underskriv** | skal_accepteres | ukendt |  | https://www.requesty.ai/dpa |
| Tavily | **acceptér/underskriv** | skal_accepteres | SCC |  | https://www.tavily.com/privacy |
| Brave Search | **ingen handling** | ikke_databehandler | SCC |  | https://cdn.search.brave.com/search-api/web/v1/client/_app/immutable/assets/brave-search-api-dpa-2025-09-09.DRXCoye6.pdf |
| Perplexity | **undersøg** | ukendt | DPF+SCC |  | https://www.perplexity.ai/hub/legal/dpa |
| Bunny.net | **acceptér/underskriv** | skal_accepteres | EU |  | https://bunny.net/gdpr/ |
| Backblaze B2 | **link** | auto | DPF+SCC |  | https://www.backblaze.com/company/policy/dpa-for-eea-eu-residents |
| Scaleway | **link** | auto | EU |  | https://www-uploads.scaleway.com/DPA_2024_ENG_b0abb5cc26.pdf |
| Hetzner | **acceptér/underskriv** | skal_accepteres | EU |  | https://docs.hetzner.com/general/company-and-policy/data-protection-at-hetzner/ |
| Forward Email | **link** | auto | SCC |  | https://forwardemail.net/en/dpa |
| inMobile | **acceptér/underskriv** | skal_accepteres | EU |  | https://www.inmobile.com/en/data-security-compliance |
| QuickPay | **acceptér/underskriv** | skal_accepteres | EU |  | https://learn.quickpay.net/public/helpdesk/files/data_processing-agreement.pdf |
| Polar | **link** | auto | SCC |  | https://polar.sh/legal/data-processing-addendum |
| Flexprice | **link** | auto | ukendt |  | https://flexprice.io/data-handling |
| Piwik PRO | **acceptér/underskriv** | skal_accepteres | EU |  | https://piwik.pro/wp-content/uploads/2025/08/Piwik-PRO-Business-Data-Agreement.pdf |
| Google Analytics | **link** | auto | DPF+SCC |  | https://support.google.com/analytics/answer/3379636?hl=en |
| Slack | **acceptér/underskriv** | skal_accepteres | DPF+SCC |  | https://slack.com/terms-of-service/data-processing |
| Discord | **ingen handling** | ikke_databehandler | DPF+SCC |  | https://support.discord.com/hc/en-us/articles/37891902561687-Data-Processing-Agreement-Discord-as-a-Processor |
| Gravatar | **ingen handling** | ikke_databehandler | DPF+SCC |  | https://automattic.com/privacy/ |
| Vimeo | **acceptér/underskriv** | skal_accepteres | DPF+SCC | JA (sanneandersen) | https://help.vimeo.com/hc/en-us/articles/18332128580241-Does-Vimeo-offer-a-Data-Processing-Agreement |

## Handlinger for leverandører i DRIFT (grundlag for handlingslisten, F201.3)

Målt på 34 kørende Fly-apps (nøgle-NAVNE + målte endpoints).

| leverandør | anbefaling | apps i drift | hvad |
|---|---|---|---|
| Fly.io | **acceptér/underskriv** | auto-scaling-container-app, broberg-ai, broberg-autodoc, broberg-discovery, broberg-helpdesk-api, broberg-helpdesk-console, broberg-helpdesk-db, broberg-id, broberg-services, buddy-brain, buddy-cloud, buddy-edge-fly, buddy-voice, cardmem, cardmem-lens, dns-mcp, fd-sport, fd-sundhed, moovyy, pitch-vault, sanneandersen-services, sanneandersen-site, trail-admin, trail-db-001, trail-engine-001, trail-widget, upmetrics, webhouse-app, webhouse-contract-manager, webhouse-cronjobs, webhouse-dk, webhouse-whapi, webhouse-whop, xrt81 | Sign in → Documents (fly.io/documents) → sign the DPA (pre-signed by Fly.io, active once customer signs) |
| Tigris | **link** | cardmem, trail-db-001, trail-engine-001, upmetrics |  |
| Cloudflare | **link** | buddy-brain, buddy-cloud, buddy-edge-fly, cardmem, cardmem-lens, dns-mcp, fd-sundhed, sanneandersen-site, webhouse-app, webhouse-contract-manager, webhouse-dk, xrt81 |  |
| Resend | **link** | broberg-ai, broberg-helpdesk-api, broberg-id, cardmem, fd-sundhed, moovyy, pitch-vault, sanneandersen-site, trail-admin, upmetrics, webhouse-contract-manager, webhouse-cronjobs, webhouse-whapi, webhouse-whop, xrt81 |  |
| Stripe | **link** | sanneandersen-site |  |
| Supabase | **link** | broberg-id, fd-sundhed, webhouse-whop |  |
| Turso | **acceptér/underskriv** | broberg-discovery, buddy-cloud | Log in → Documents section → execute the Data Processing Agreement (paid subscribers only; blog says Scaler/Pro/Enterprise) |
| Mistral AI | **link** | broberg-ai, broberg-autodoc, broberg-helpdesk-api, buddy-voice, cardmem, fd-sundhed, pitch-vault, sanneandersen-site, trail-engine-001, webhouse-app, xrt81 |  |
| OpenAI | **link** | xrt81 |  |
| Anthropic | **link** | webhouse-whop, xrt81 |  |
| Google (Gemini/Vertex AI) | **link** | webhouse-app |  |
| OpenRouter | **link** | buddy-brain, buddy-edge-fly, cardmem, sanneandersen-site, trail-engine-001, upmetrics, xrt81 |  |
| ElevenLabs | **link** | webhouse-app |  |
| Microsoft Azure | **link** | broberg-ai, buddy-voice, cardmem |  |
| GitHub | **link** | broberg-autodoc, broberg-id, buddy-edge-fly, cardmem, trail-admin, upmetrics, webhouse-app, webhouse-contract-manager, webhouse-whapi |  |
| Google (Workspace/Gmail/Calendar/OAuth) | **acceptér/underskriv** | broberg-id, cardmem, moovyy, sanneandersen-site, trail-admin, xrt81 | Admin console (super admin) → Menu → Account → Account settings → Legal and compliance → Security and Privacy Additional Terms → Cloud Data Processing Addendum → Review and Accept |
| Google Firebase (FCM) | **link** | fd-sundhed |  |
| Apple (APNs/Sign in/App Store) | **ingen handling** | broberg-id, xrt81 |  |
| Web Push (browserens push-tjeneste) | **ingen handling** | cardmem, fd-sundhed, trail-engine-001, webhouse-app, xrt81 |  |
| LinkedIn | **ingen handling** | broberg-id |  |
| Microsoft (login) | **ingen handling** | broberg-id |  |
| Simply.com | **acceptér/underskriv** | dns-mcp | Simply.com Kontrolpanel (logged in) → find the databehandleraftale and sign it there |
| GatewayAPI | **acceptér/underskriv** | broberg-id | DPA takes effect on both parties' signature; how to sign isn't stated online, so check the GatewayAPI dashboard or ask support |
| Complimenta | **ingen handling** | fd-sundhed |  |
| Opkald.ai | **acceptér/underskriv** | fd-sundhed | No public DPA document; request it from hej@opkald.ai |
| TMDB | **ingen handling** | moovyy |  |
| Upstash | **link** | auto-scaling-container-app |  |
| fal.ai | **link** | sanneandersen-site |  |
| Hugging Face | **acceptér/underskriv** | broberg-services | Get an Enterprise Hub or Enterprise plan (huggingface.co/enterprise); the GDPR DPA is offered only through that subscription. |
| RunPod | **acceptér/underskriv** | broberg-services | Fill in the signature block and required information in the DPA and submit it via the link on the DPA page; it binds only after that. |
| Discord | **ingen handling** | cardmem, webhouse-cronjobs, webhouse-whop |  |
| Vimeo | **acceptér/underskriv** | sanneandersen-site | DPA only on Enterprise/OTT plans (https://vimeo.com/legal/enterprise-terms/dpa); self-serve plans get none, but embed users can sign controller-to-controller SCCs via Vimeo's webform (https://www.vimeo.axdraft.com). |

## Ukendte udgående værter (skal vurderes — kan være en overset leverandør)

- `aistudio.google.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:70; webhousecode/cms packages/cms-admin/src/app/admin/(workspace)/organizations/settings/page.tsx:394
- `andet.dk` — broberg-ai/helpdesk apps/core/src/lookup/katalog.ts:104
- `apps.apple.com` — broberg-ai/fd-sundhed apps/web/public/qr/MANIFEST.json:13; webhousecode/fysiodk-aalborg-sport apps/web/src/app/(app)/app/admin/kommunikation/campaign-form.tsx:49
- `archive.debian.org` — broberg-ai/trail apps/db/Dockerfile:42
- `artificialanalysis.ai` — broberg-ai/ai-sdk inventory.json:4798
- `barock.dev` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `beacon.embedded` — broberg-ai/beacon packages/api/src/embed.ts:57
- `benalman.com` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:148
- `bit.ly` — cbroberg/moovyy package-lock.json:4140
- `brave.com` — webhousecode/cms packages/cms-admin/src/app/admin/(workspace)/organizations/settings/page.tsx:418
- `buddycloud.cc` — webhousecode/buddy .github/workflows/deploy-cloud-fly.yml:122; broberg-ai/cardmem apps/server/src/cross-edge-dispatch.ts:11; broberg-ai/components packages/seti-client/src/client.ts:13
- `bun.sh` — webhousecode/buddy scripts/edge/buddy-edge-install.sh:124; broberg-ai/cardmem apps/lens-cloud/Dockerfile:24; webhousecode/cms packages/cms-admin/builder/Dockerfile:50
- `capacitorjs.com` — broberg-ai/fd-sundhed apps/web/ios/App/App/AppDelegate.swift:41; webhousecode/fysiodk-aalborg-sport apps/web/ios/App/App/AppDelegate.swift:41
- `cbroberg.github.io` — webhousecode/cms scripts/generate-example-readmes.ts:133; broberg-ai/trail apps/landing/_data/deploy-log.json:232
- `cherne.net` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:139
- `claude.ai` — webhousecode/buddy docker/edge/Dockerfile:34; broberg-ai/cardmem apps/server/src/api/mcp-config.ts:30
- `claudefa.st` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `cloudflare-dns.com` — webhousecode/dns-mcp src/tools/domain-tools.ts:116
- `code.claude.com` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `collider.com` — cbroberg/moovyy watch-service/news-sources.js:35
- `console.aws.amazon.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:90
- `console.cloud.google.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:93; webhousecode/cms packages/cms-admin/src/app/admin/(workspace)/lighthouse/page.tsx:657; cbroberg/coverletter-generator google-drive-uploader/authenticate.js:31
- `d.android.com` — webhousecode/cms packages/cms-mobile/android/app/src/androidTest/java/com/getcapacitor/myapp/ExampleInstrumentedTest.java:14; broberg-ai/fd-sundhed apps/web/android/app/src/androidTest/java/com/getcapacitor/myapp/ExampleInstrumentedTest.java:14; webhousecode/fysiodk-aalborg-sport apps/web/android/app/src/androidTest/java/com/getcapacitor/myapp/ExampleInstrumentedTest.java:14
- `ddz.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:338
- `deadline.com` — cbroberg/moovyy watch-service/news-sources.js:26
- `developers.meethue.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:101
- `difzt.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/behandlerhuset-1-sal/behandlerhuset-1-sal.htm:344
- `discord.gg` — webhousecode/cms packages/cms-admin/src/components/help-drawer.tsx:207
- `discovery.meethue.com` — broberg-ai/beacon packages/core/src/bridge/discovery.ts:29
- `dk.rs-online.com` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `docs.m5stack.com` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `dotenvx.com` — webhousecode/dns-mcp package-lock.json:391; cbroberg/moovyy package-lock.json:3431
- `download.pytorch.org` — broberg-ai/voice-engine Dockerfile:141
- `editorial.rottentomatoes.com` — cbroberg/moovyy watch-service/news-sources.js:41
- `eksempel-der-ikke-er-tilladt.dk` — broberg-ai/broberg-id scripts/fikstur-app.ts:119
- `eslint.org` — cbroberg/coverletter-generator package-lock.json:6779; broberg-ai/fd-sundhed pnpm-lock.yaml:3270; webhousecode/fysiodk-aalborg-sport pnpm-lock.yaml:2660
- `evil.com` — webhousecode/cms packages/cms-admin/src/app/admin/switch/[slug]/route.ts:47
- `evil.dk` — broberg-ai/broberg-id scripts/security-gate.ts:189; broberg-ai/components packages/sso/src/hono.ts:94
- `falsk-login.dk` — broberg-ai/fd-sundhed apps/web/src/app/api/godkender/vink/route.ts:152
- `fdsundhed.dk` — broberg-ai/fd-sundhed apps/web/src/app/(app)/app/platformen-bag/page.tsx:118; broberg-ai/helpdesk mockups/helpdesk-widget-paa-kundesite.html:267
- `feeds.feedburner.com` — cbroberg/moovyy watch-service/news-sources.js:45
- `feross.org` — cbroberg/coverletter-generator package-lock.json:5163; cbroberg/moovyy package-lock.json:2308; cbroberg/pitch package-lock.json:5095
- `foundation.zurb.com` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:88
- `fsl.software` — broberg-ai/cardmem packages/shared/src/licenses.ts:39
- `gitlab.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:96
- `gls.dk` — webhousecode/sanneandersen site/src/app/api/shop/pakkeshop-lookup/route.ts:24
- `goldenwingschigong.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/behandlerhuset-1-sal/behandlerhuset-1-sal.htm:346
- `house-of-wellness.dk` — broberg-ai/house-of-wellness mail/skabeloner.mjs:13
- `icimh.org` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:356
- `icr-reflexology.org` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:346
- `incompetech.com` — broberg-ai/contentpush apps/web/src/server/lib/musicShelf.ts:12
- `iscmr.org` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:354
- `jquery.com` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:7
- `jquery.org` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:14
- `json.schemastore.org` — broberg-ai/fdaa tsconfig.base.json:2; broberg-ai/upmetrics tsconfig.base.json:2
- `ko-fi.com` — broberg-ai/ai-sdk inventory.json:11923; cbroberg/moovyy package-lock.json:4143; cbroberg/pitch package-lock.json:3854
- `kpo.naevneneshus.dk` — webhousecode/cms packages/cms-admin/src/lib/link-check-probe.ts:74; webhousecode/sanneandersen site/content/sider-content/handelsbetingelser-en.json:11
- `kundens-endepunkt.dk` — broberg-ai/helpdesk apps/console/src/skal/paneler/applikationer.tsx:366
- `lametric-documentation.readthedocs.io` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `letterboxd.com` — cbroberg/moovyy watch-service/news-sources.js:42
- `lh3.googleusercontent.com` — broberg-ai/broberg-id src/security-headers.ts:67
- `localhost.angriber.dk` — broberg-ai/broberg-id src/stub-oauth.ts:53
- `lucide.dev` — webhousecode/cms packages/cms-inline-edit/src/index.ts:332
- `lwlies.com` — cbroberg/moovyy watch-service/news-sources.js:47
- `maps.apple.com` — broberg-ai/xrt81 packages/shared/src/maps.ts:43
- `mb.org` — broberg-ai/trail apps/server/scripts/verify-neuroncount-filnavn.ts:47
- `media.licdn.com` — broberg-ai/broberg-id src/security-headers.ts:68
- `modernizr.com` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:2
- `moovyy.com` — cbroberg/moovyy cloud/fly.toml:24
- `mths.be` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:96
- `nifab.no` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:350
- `nordjyske.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/index.html:334
- `ondt-site.dk` — webhousecode/sanneandersen site/src/lib/auth/safe-next.ts:13
- `ondt.dk` — broberg-ai/fd-sundhed apps/web/scripts/booking-i-produktionen.mjs:60
- `opencode.ai` — broberg-ai/openbuddy opencode.json:2
- `opencollective.com` — cbroberg/coverletter-generator package-lock.json:1199; webhousecode/dns-mcp package-lock.json:244; cbroberg/moovyy package-lock.json:90
- `other.host` — broberg-ai/broberg-id src/verify-live.ts:16
- `pagesix.com` — cbroberg/moovyy watch-service/news-sources.js:53
- `passets-cdn.pinterest.com` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_medarbejdere_christian_broberg.htm.html:180
- `paulmillr.com` — cbroberg/coverletter-generator package-lock.json:5601; cbroberg/pitch package-lock.json:5414
- `paypal.me` — cbroberg/coverletter-generator package-lock.json:7385; cbroberg/moovyy package-lock.json:3984; cbroberg/pitch cli/package-lock.json:750
- `pinterest.com` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_medarbejdere_christian_broberg.htm.html:180
- `plus.google.com` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/aabningstider/aabningstider.htm:136
- `politiken.dk` — cbroberg/moovyy watch-service/news-sources.js:65
- `poolside.ai` — broberg-ai/ai-sdk inventory.json:1692
- `reflexology-europe.org` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:344
- `resend.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:80; webhousecode/cms packages/cms-admin/src/app/admin/(workspace)/organizations/settings/page.tsx:508
- `s7.addthis.com` — webhousecode/buddy docker/old-site/arkiv/_.html:119
- `schemas.xmlsoap.org` — webhousecode/sanneandersen site/src/app/api/shop/pakkeshop-lookup/route.ts:66
- `screenrant.com` — cbroberg/moovyy watch-service/news-sources.js:36
- `shelly-api-docs.shelly.cloud` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `sizzlejs.com` — webhousecode/buddy docker/old-site/arkiv/aktiver/js/foundation.js:10
- `soundvenue.com` — cbroberg/moovyy watch-service/news-sources.js:60
- `sqlite.org` — broberg-ai/trail packages/db/src/backup.ts:11
- `status.claude.com` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `stub.fal` — broberg-ai/ai-sdk src/providers/stub.ts:122
- `t2.gstatic.com` — cbroberg/coverletter-generator lib/image-search/google-images-scraper.ts:81
- `thariqs.github.io` — broberg-ai/cardmem apps/web/src/components/board.tsx:2
- `thepiratebay.org` — cbroberg/moovyy server.js:847
- `tidelift.com` — cbroberg/coverletter-generator package-lock.json:5438; cbroberg/moovyy package-lock.json:2638; cbroberg/pitch package-lock.json:5343
- `tsx.hirok.io` — broberg-ai/beacon pnpm-lock.yaml:154; webhousecode/buddy pnpm-lock.yaml:427
- `tsx.is` — broberg-ai/cardmem pnpm-lock.yaml:736; broberg-ai/contract-manager pnpm-lock.yaml:208; cbroberg/coverletter-generator package-lock.json:312
- `turbo.build` — broberg-ai/beacon turbo.json:2; webhousecode/buddy turbo.json:2; webhousecode/cms turbo.json:2
- `turborepo.com` — broberg-ai/cardmem turbo.json:2; broberg-ai/helpdesk turbo.json:2
- `ui.shadcn.com` — webhousecode/cms packages/cms-admin/components.json:2; broberg-ai/contract-manager components.json:2; cbroberg/coverletter-generator components.json:2
- `uit.no` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:348
- `v8.dev` — cbroberg/moovyy package-lock.json:6837
- `vaert.dk` — broberg-ai/helpdesk apps/core/src/access/widget-tokens.ts:34
- `variety.com` — cbroberg/moovyy watch-service/news-sources.js:27
- `virksomhed.dk` — cbroberg/coverletter-generator app/(app)/generate/page.tsx:2232
- `vnleker.no` — webhousecode/vnlekerv2 src/mail/webhouse-mail.mjs:90
- `widgets.klout.com` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_the_staff_christian_broberg.htm.html:180
- `www.1337xx.to` — cbroberg/moovyy server.js:846
- `www.aarhushavn.dk` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_fakta_og_historie_.html:176
- `www.addthis.com` — webhousecode/buddy docker/old-site/arkiv/_.html:119
- `www.apache.org` — broberg-ai/cardmem packages/shared/src/licenses.ts:273
- `www.apple.com` — broberg-ai/beacon scripts/launchd.sh:49; webhousecode/buddy packages/cli/src/commands/daemon.ts:23; broberg-ai/cardmem apps/agent/scripts/install-launchd.sh:33
- `www.atea.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.brainsbusiness.dk` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_partnere_.html:165
- `www.clienti.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.computerworld.dk` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_nyheder_ipad_salgs_applikationer_til_din_virksomhed.htm.html:153
- `www.cowi.dk` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_partnere_.html:154
- `www.dao.as` — webhousecode/sanneandersen site/src/lib/mail-templates/shop-shipped.ts:11
- `www.difzt.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/behandlerhuset-1-sal/behandlerhuset-1-sal.htm:345
- `www.dr.dk` — cbroberg/moovyy watch-service/news-sources.js:62
- `www.eonline.com` — cbroberg/moovyy watch-service/news-sources.js:50
- `www.fdz.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:336
- `www.fiels.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.filmz.dk` — cbroberg/moovyy watch-service/news-sources.js:59
- `www.friendfeed.com` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_medarbejdere_christian_broberg.htm.html:176
- `www.gamesradar.com` — cbroberg/moovyy watch-service/news-sources.js:38
- `www.gls.dk` — webhousecode/sanneandersen site/src/app/api/shop/pakkeshop-lookup/route.ts:5
- `www.goldenwingschigong.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/kurser_02/index.html:322
- `www.google.com` — webhousecode/buddy docker/old-site/arkiv/_dk_kontakt_kontakt_webhouse_.html:136; cbroberg/coverletter-generator lib/utils/company-website-search.ts:27; cbroberg/moovyy server.js:849
- `www.historyofinformation.com` — broberg-ai/trail apps/landing/_data/link-check-result.json:60
- `www.hollywoodreporter.com` — cbroberg/moovyy watch-service/news-sources.js:29
- `www.indiewire.com` — cbroberg/moovyy watch-service/news-sources.js:30
- `www.integrativecare.se` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:352
- `www.ipaper.dk` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_partnere_.html:158
- `www.jobindex.dk` — cbroberg/coverletter-generator lib/scraper/jobindex-scraper.ts:77
- `www.justjared.com` — cbroberg/moovyy watch-service/news-sources.js:51
- `www.konxion.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.leilaeriksen.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:342
- `www.macrumors.com` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `www.milonic.com` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/js/milonic_src.js:6
- `www.moovyy.com` — cbroberg/moovyy cloud/fly.toml:33
- `www.ncbi.nlm.nih.gov` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:358
- `www.npmjs.com` — broberg-ai/cardmem apps/server/src/api/vault.ts:79; webhousecode/cms landing/webhouse-app-landing.html:829; broberg-ai/components scripts/inventory-data.mjs:398
- `www.odeum.com` — webhousecode/buddy docker/old-site/arkiv/_dk_webhouse_nyheder_nyt_odeum_cms_logo.htm.html:123
- `www.patreon.com` — cbroberg/coverletter-generator package-lock.json:5159; cbroberg/moovyy package-lock.json:2304; cbroberg/pitch package-lock.json:5091
- `www.polygon.com` — cbroberg/moovyy watch-service/news-sources.js:37
- `www.pricerunner.dk` — webhousecode/buddy apps/server/src/harvest/__fixtures__/ture.json:1
- `www.rogerebert.com` — cbroberg/moovyy watch-service/news-sources.js:46
- `www.scanimex.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.sitemaps.org` — broberg-ai/broberg-ai-site src/sitemap.ts:30; webhousecode/cms packages/cms-admin/src/lib/post-build-enrich.ts:496
- `www.slashfilm.com` — cbroberg/moovyy watch-service/news-sources.js:34
- `www.srab.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:340
- `www.theatlantic.com` — broberg-ai/trail apps/landing/_data/link-check-result.json:24
- `www.thewrap.com` — cbroberg/moovyy watch-service/news-sources.js:31
- `www.thimm.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.virgil.dk` — webhousecode/buddy docker/old-site/arkiv/_en_webhouse_partners_.html:153
- `www.youtube-nocookie.com` — cbroberg/moovyy cloud/src/web/app.js:578
- `www.zct.dk` — webhousecode/sanneandersen arkiv/www.sanneandersen.dk/dk/om_sanne/samarbejdspartnere/index.html:334
- `x.dk` — webhousecode/cms packages/cms-admin/src/lib/link-check-classify.ts:58; broberg-ai/components packages/mail-core/verify-no-drift.mjs:29; broberg-ai/trail packages/shared/scripts/verify-wiki-links.ts:186
- `yourwebsite.com` — cbroberg/coverletter-generator app/(app)/settings/page.tsx:551
- `yts.mx` — cbroberg/moovyy server.js:848
- `zod.dev` — broberg-ai/trail packages/shared/openapi.yaml:108

## Krydstjek mod discovery.broberg.ai/api/infra

- Fly.io (`fly`): fundet i scanningen
- Cloudflare (`cloudflare`): fundet i scanningen
- Resend (`resend`): fundet i scanningen
- Stripe (`stripe`): fundet i scanningen
- Supabase (`supabase`): fundet i scanningen
- Turso / libSQL (`turso`): fundet i scanningen
- npm / OIDC publishing (`npm`): fundet i scanningen
- Pitch Vault (`pitch`): ikke med — vores egen tjeneste (Pitch Vault), hostet på Fly
- Image processing (sharp / HEIC) (`image-processing`): ikke med — bibliotek (sharp) der kører på vores egne maskiner — ingen tredjepart
- GitHub Actions / CI-CD (`github-actions`): ikke med — del af GitHub, som står på listen
- Frontend (Preact / Next / PWA / web) (`frontend`): ikke med — kategori af browser-kode, ikke en leverandør
- AI / LLM providers (@broberg/ai-sdk) (`ai-providers`): ikke med — kategori; de enkelte AI-leverandører står på listen hver for sig
- Fleet ops (cc-session auth · edge · inter-session comms) (`fleet-ops`): ikke med — vores egen drift af udviklingsmaskiner, ingen kundedata
- Document generation (PDF) (`documents`): ikke med — PDF-generering på vores egne maskiner — ingen tredjepart
- Native macOS/iOS apps (Swift — codesign, entitlements, TCC) (`macos-native`): ikke med — kategori (app-signering); Apple står på listen
