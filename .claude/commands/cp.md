---
description: Commit the working tree AND push to origin main — Christian's "cp" (alias of /commit+push).
...
Commit the current working tree **and push to origin main** — Christian's `cp` (same behaviour as `/commit+push`).
